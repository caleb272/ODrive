...
bool CANUavcan::renew_subscription(size_t i) {
    ...
    return canbus_->subscribe(filter, [](void* ctx, const can_Message_t& msg) {
        ((CANUavcan*)ctx)->handle_can_message(msg);
    }, this, &subscription_handles_[i]);
}

void CANUavcan::handle_can_message(const can_Message_t& msg) {
    //     Frame
    // nodeID | CMD
    // 6 bits | 5 bits
...
void CANUavcan::do_command(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::nmt_callback(const Axis& axis, const can_Message_t& msg) {
    // Not implemented
}

void CANUavcan::estop_callback(Axis& axis, const can_Message_t& msg) {
    axis.error_ |= Axis::ERROR_ESTOP_REQUESTED;
}

bool CANUavcan::get_motor_error_callback(const Axis& axis) {
    can_Message_t txmsg;
...
bool CANUavcan::get_encoder_error_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
...
bool CANUavcan::get_sensorless_error_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
...
void CANUavcan::set_axis_nodeid_callback(Axis& axis, const can_Message_t& msg) {
    axis.config_.can.node_id = can_getSignal<uint32_t>(msg, 0, 32, true);
}

void CANUavcan::set_axis_requested_state_callback(Axis& axis, const can_Message_t& msg) {
    axis.requested_state_ = static_cast<Axis::AxisState>(can_getSignal<int32_t>(msg, 0, 16, true));
}

void CANUavcan::set_axis_startup_config_callback(Axis& axis, const can_Message_t& msg) {
    // Not Implemented
}

bool CANUavcan::get_encoder_estimates_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
...
bool CANUavcan::get_sensorless_estimates_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
...
bool CANUavcan::get_encoder_count_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
...
void CANUavcan::set_input_pos_callback(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::set_input_vel_callback(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::set_input_torque_callback(Axis& axis, const can_Message_t& msg) {
    axis.controller_.input_torque_ = can_getSignal<float>(msg, 0, 32, true);
}

void CANUavcan::set_controller_modes_callback(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::set_limits_callback(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::start_anticogging_callback(const Axis& axis, const can_Message_t& msg) {
    axis.controller_.start_anticogging_calibration();
}

void CANUavcan::set_traj_vel_limit_callback(Axis& axis, const can_Message_t& msg) {
    axis.trap_traj_.config_.vel_limit = can_getSignal<float>(msg, 0, 32, true);
}

void CANUavcan::set_traj_accel_limits_callback(Axis& axis, const can_Message_t& msg) {
...
void CANUavcan::set_traj_inertia_callback(Axis& axis, const can_Message_t& msg) {
    axis.controller_.config_.inertia = can_getSignal<float>(msg, 0, 32, true);
}

void CANUavcan::set_linear_count_callback(Axis& axis, const can_Message_t& msg){
    axis.encoder_.set_linear_count(can_getSignal<int32_t>(msg, 0, 32, true));
}

bool CANUavcan::get_iq_callback(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
    txmsg.id += MSG_GET_IQ;
    txmsg.isExt = axis.config_.can.is_extended;
...
bool CANUavcan::get_vbus_voltage_callback(const Axis& axis) {
    can_Message_t txmsg;
...
void CANUavcan::clear_errors_callback(Axis& axis, const can_Message_t& msg) {
    odrv.clear_errors(); // TODO: might want to clear axis errors only
}

uint32_t CANUavcan::service_stack() {
...
bool CANUavcan::send_heartbeat(const Axis& axis) {
    can_Message_t txmsg;
    txmsg.id = axis.config_.can.node_id << NUM_CMD_ID_BITS;
